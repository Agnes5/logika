 ...
 *  structure file
 *)
structure id283219 :> PART_ONE =
struct
  exception NotImplemented

  datatype 'a tree= Leaf of 'a | Node of 'a tree * 'a * 'a tree

  fun sum n = if n > 0 then n + sum(n-1) 
  				else 0;

  fun fac n = if n > 0 then n * fac(n-1) 
  				else 1;

  fun fib n = if n > 1 then fib(n-1) + fib(n-2)
              	else if n = 1 then 1 
              	else 1;
  ...
  fun depth (t:'a tree) = case t of
  							Leaf l => 0
  							| Node(l, _, p) =>
  							if depth l > depth p then depth l + 1
  							else depth p + 1;

  fun binSearch (t:int tree) (x:int) = case t of
  							Leaf l => l = x
  							| Node(l, v, r) =>
  							if x = v then true
  ...
  fun listAdd (a:int list) [] = a
  	...
  	| listAdd (a: int list as ha :: ta) (b: int list as hb :: tb) = (ha + hb) :: listAdd ta tb;

  fun insert (m:int) [] = [m]
  	| insert (m:int) (l:int list as h :: t) = if m < h then m :: l
  											  else h :: insert m t; 

  fun insort (l:int list) = let
  								fun sort [] res = res
  ...
  fun lall _ [] = true
  	| lall f (x::xs) = if (f x) then (lall f xs)
  						else false; 
